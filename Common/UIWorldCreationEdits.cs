	...
	internal class UIWorldCreationEdits {
		...
		public static void UIWorldCreation_BuildPage(On_UIWorldCreation.orig_BuildPage orig, UIWorldCreation self) {
			...
			#region Ore UI List
			{
				...
				UIElement innerList = (UIElement)typeof(UIList).GetField("_innerList", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(_oreList);
			...
			#region Biome UI List
			{
				...
				UIElement innerList = (UIElement)typeof(UIList).GetField("_innerList", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(_biomeList);
				foreach (UIElement item in items) {
					innerList.Append(item);
				}
				innerList.Recalculate();
		...
		private static void UIWorldCreation_Draw(ILContext il) {
			...
			c.EmitDelegate<Action<UIWorldCreation>>((self) => {
				seed = (string)typeof(UIWorldCreation).GetField("_optionSeed", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(self);
			});
		}
		...
		private static void MakesWorldsUnplayable(On_UIWorldListItem.orig_PlayGame orig, UIWorldListItem self, UIMouseEvent evt, UIElement listeningElement) {
			if ((WorldFileData)typeof(AWorldListItem).GetField("_data", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(self) == null)
				return;
