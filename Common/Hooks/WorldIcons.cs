...
namespace AltLibrary.Common.Hooks
{
	internal static class WorldIcons
	{
		...
		public static void Init()
		{
			...
			EditsHelper.On<AWorldListItem>(typeof(AWorldListItem).GetMethod("GetIconElement", BindingFlags.Instance | BindingFlags.NonPublic).Name, AWorldListItem_GetIconElement);
			WarnUpdate = 0;
		...
		private static void UIWorldListItem_ctor(ILContext il)
		{
			...
			{
				WorldFileData data = (WorldFileData)typeof(AWorldListItem).GetField("_data", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(uiWorldListItem);
