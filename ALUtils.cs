...
namespace AltLibrary {
	internal static class ALUtils {
		...
		public static bool IsWorldValid(UIWorldListItem self) {
			GetWorldData((WorldFileData)typeof(AWorldListItem).GetField("_data", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(self), out Dictionary<string, AltLibraryConfig.WorldDataValues> tempDict, out string path2);
			bool valid = true;
